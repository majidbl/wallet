...
const (
	...
	ErrInvalidWallet    = "Invalid wallet"
...
var (
	...
	ExistsWalletError     = errors.New("User with given wallet already exists")
...
func parseValidatorError(err error) RestErr {
	...
	if strings.Contains(strings.ToLower(err.Error()), "wallet") {
		return NewRestError(http.StatusBadRequest, ErrInvalidWallet, err)
	}
